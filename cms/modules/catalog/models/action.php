...
class CatalogModel_Action extends Model_Base
{
     ...
     * @return array массив, где индекс 'count' - количество дочерних элементов, 'limit' - подстрока limit sql запроса
     */
    public function getPositionPagerArray($table_name, $section_id, $page_size, $where=null, $where_replace=null)
    {        
        $page = Utils::getGET("page") ? Utils::getGET("page") : 1;
        $page = $page < 1 ? 1 : $page;
        if ( $where_replace )
        {
            $where = $where_replace;
            $rows = $this->getSectionTable()->select('select count(*) as count from '.$table_name.' where ' . $where);
        }
        else
        {
	...
	public function getSectionData($name)
    {
		...
		$id = $section->id;	
		$config = Config::__("catalog");
		$section_table_name =  $config->tables_section[$section->section_table]['table'];		
		
        $rows = $this->getSectionTable()->select(
				"SELECT s1.title, s1.alias, s1.position_table, s2.* "
				. " FROM catalog_section s1 " 
				. " LEFT JOIN $section_table_name s2 ON s1.id = s2.id " 
				. " WHERE s1.id = :id"
	...
	private function chainSection($id, $arRes ,$i)
    {
        $section = $this->getSection($id);        
        if($section->parent_id)
        {
            $arRes[ $i ]["alias"] = $section->alias;
			$arRes[ $i ]["title"] = $section->title;
			$arRes[ $i ]["id"] = $section->id;
            $i = $i + 1;
            $arRes = $this->chainSection($section->parent_id, $arRes, $i);
        }      
        return $arRes;
    }
