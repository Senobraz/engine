                                    ...
                                    if(form.getForm().isValid())
                                    {
                                        form.getForm().submit({
                                            ...
                                            success: function(form, action)
                                            {
                                                ...
                                                App.showEditor({
                                                        url: '/ajax/ext/feed.ext.editor?id='+action.result.id,
        ...
        <?php if($args->top) echo 'render'; else echo 'expand' ?> : function(panel,anim)
        {
            if(panel.firstexp != true)
            {
                var tree = new Ext.tree.TreePanel({
                    ...
                    autoScroll: true,
                    rootVisible: false,
                    dataUrl: '/ajax/ext/feed.ext.tree',
                    ...
                    listeners :
                    {
                        click : function(node)
                        {
                            ...
                            if(node.id > 0)
                            {
                                dis = false;
                                App.showEditor({
                                    url: '/ajax/ext/feed.ext.editor?id='+node.id,
