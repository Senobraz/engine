...
//    }
    ...
    public function show()
    {
            ...
            //if($check)
            {
                ...
                foreach($mails as $mail)
                {
                    ...
                    foreach($field_rows as $row)
                    {
						...
						{
							$error_message .= 'Превышен допустимый размер: '.$row['name']."\n";
						...
						if($row['valid_name'] == 1 && !$res['errors'][$row['nameid']])
                        {                           
							if( is_numeric($value) || ( mb_strlen($value) < 2  )  )
							{
								$error_message .= 'Некорректно заполнено поле: '.$row['name']."\n";
                                $res['errors'][$row['nameid']] = $row['name'];
                                $check = false;	
							}
                        }	
						...
						if($row['valid_phone'] == 1 && $res['errors'][$row['nameid']])
                        {                           
							$regular = "#^\+[0-9]{1,2}\s?\([0-9]{3}\)\s?[0-9]+\-[0-9]+\-[0-9]{2}+$#";
                            
                            if ( !empty($row["regular_phone"]))
                                $regular = $row["regular_phone"];
        ...
        if(Utils::GetPost('submit_'.$form->id) && Utils::GetPost('ajax_form')==1){
            ...
            {
                $res["result"] = "success";
                $res['message'] = nl2br($success_message);
            }
