    ...
    tbar:
    {
        ...
        [
                                ...
                                Ext.MessageBox.prompt('Создать пункт меню', 'Введите заголовок пункта меню:',
                                    function(btn, textpromt)
                                    {
                                        if(btn == 'ok')
                                        {
                                            ...
                                            Ext.Ajax.request(
                                            {
                                                url : '/ajax/ext/menus.ext.add_item',
                                                method: 'POST',
                                                params:
                                                {
                                                    title: textpromt,
                    ...
                    Ext.MessageBox.confirm('Подтверждение', 'Вы действительно хотите удалить пункт меню: '+item.node.text+'?',
                        function(btn)
                        {
                            ...
                            if(btn == 'yes')
                            {
                                Ext.Ajax.request(
                                    {
                                        url : '/ajax/ext/menus.ext.delete_item',
                                        method: 'POST',
                                        params:
                                        {
                                            id: node.id
            ...
            },
            {
                ...
                {
                    Ext.Ajax.request({
                        url : '/ajax/ext/menus.ext.form_menu',
                        method: 'POST',
                        params:
                        {
                            id: <?php echo $menu->id ?>,
            ...
            [
                {
                    ...
                    loader: new Ext.tree.TreeLoader(
                    {
                        ...
                        dataUrl: '/ajax/ext/menus.ext.tree_items'
                    }),
                    ...
                    listeners:
                    {
                            ...
                            else
                            {
                                tabs.add({
                                    ...
                                    listeners:
                                    {
                                        ...
                                        {
                                            Ext.Ajax.request({
                                                url : '/ajax/ext/menus.ext.editor_item',
                                                method: 'POST',
                                                params:
                                                {
                                                    id: node.id,
                        ...
                        beforemovenode: function(tree, node, oldParent, newParent, index)
                        {
                            if(confirm('Сохранить измененый порядок?'))
                            {
                                Ext.Ajax.request(
                                    {
                                        url : '/ajax/ext/menus.ext.reorder',
                                        method: 'POST',
                                        params:
                                        {
                                            id: node.id,
