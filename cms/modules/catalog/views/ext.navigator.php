    ...
    tbar:
    {
        ...
        [
                    ...
                    [
                        {
                            ...
                            {
                                Ext.Ajax.request({
                                    url : '/ajax/ext/catalog.ext.add_section_form',
                                    method: 'POST',
                                    params:
                                    {
            ...
            }<?if(!$alias && Security_User::inRole('dev')){?>,
            {
                ...
                {
                    App.showEditor({
                        url: '/ajax/ext/catalog.config.editor',
        ...
        <?php if($args->top) echo 'render'; else echo 'expand' ?>: function(panel,anim)
        {
            if(panel.firstexp != true)
            {
                var tree = new Ext.tree.TreePanel({
                    ...
                    border: false,
                    rootVisible: false,
                    dataUrl: '/ajax/ext/catalog.ext.tree<?php if($alias) echo '?alias='.$alias ?>',
                    ...
                    listeners:
                    {
                        ...
                        {
                            App.showEditor({
                                url: '/ajax/ext/catalog.ext.editor?id='+node.id,
                        ...
                        beforemovenode: function(tree, node, oldParent, newParent, index)
                        {
                            if(confirm('Сохранить измененый порядок?'))
                            {
                                Ext.Ajax.request(
                                    {
                                        url : '/ajax/ext/catalog.ext.reorder',
                                        method: 'POST',
                                        params:
                                        {
                                            id: node.id,
